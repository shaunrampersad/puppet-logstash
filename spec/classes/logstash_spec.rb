          ...
          it do
            should contain_file('/opt/logstash')
              .with('ensure' => 'directory',
                    ...
                    'group' => 'logstash',
                    'mode' => '0750')
              .that_comes_before('Staging::File[logstash-2.0.0.tar.gz]')
          end

          it do
            should contain_staging__file('logstash-2.0.0.tar.gz')
              .with('source' => 'https://download.elastic.co/logstash/logstash/logstash-2.0.0.tar.gz')
          end

          it do
            should contain_staging__extract('logstash-2.0.0.tar.gz')
              .with('target' => '/opt/logstash',
